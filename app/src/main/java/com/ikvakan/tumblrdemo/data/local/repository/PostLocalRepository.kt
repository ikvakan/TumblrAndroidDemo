...
import com.ikvakan.tumblrdemo.util.extensions.toPostLocalDto
...
class PostLocalRepositoryImpl(private val postDao: PostDao) : PostLocalRepository {
    override suspend fun upsertPost(post: Post) = postDao.upsertPost(post.toPostLocalDto())
    override suspend fun upsertPosts(posts: List<Post>) =
        postDao.upsertPosts(posts.map { it.toPostLocalDto() })
    override suspend fun getPosts(): List<Post> = postDao.getPosts().map { it.toPost() }
    override suspend fun getFavoritePosts(): List<Post> =
        postDao.getFavoritePosts().map { it.toPost() }
